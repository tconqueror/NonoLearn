...
Manage::Manage(CServerRoomDlg* p)
{
	dlg = p;
}
...
UINT Manage::StartListen(CString s_ip, CString s_port)
{
	...
	while ((news = accept(s, (struct sockaddr*)&client, &c)) != INVALID_SOCKET)
	{
		...
		int recv_size = 0;
		CString curname;
		if ((recv_size = recv(news, buf, 10000, 0)) != SOCKET_ERROR)
		{
			int client_session = 0;
			memcpy_s(&client_session, 4, buf, 4);
			if ((curname = db->GetAccess(client_session, s_port)).GetLength() == 0)
				continue;
			if (send(news, "OK", 2, 0) == SOCKET_ERROR)
			{
				continue;
			}
			curname = CString(L"[") + curname + CString(L"]");
		}
		char* client_ip = inet_ntoa(client.sin_addr);
		cc_ip = (CString)client_ip;
		int client_port = ntohs(client.sin_port);
		cc_port.Format(_T("%d"), client_port);
		vs.push_back(psa(news, cc_ip, cc_port, curname));
