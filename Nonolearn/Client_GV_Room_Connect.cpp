...
Client_GV_Room_Connect::Client_GV_Room_Connect(Room_GV* t_p, int session, CString addr, CString port)
{
	Sleep(1000);
...
Client_GV_Room_Connect::~Client_GV_Room_Connect()
{
	closesocket(s);
	WSACleanup();
	clientroom->GetDlgItem(ID_SEND)->EnableWindow(false);
...
void Client_GV_Room_Connect::SendData(CString msg)
{
...
void Client_GV_Room_Connect::Start()
{
	int recv_size = 0;
	char buf[10000];
